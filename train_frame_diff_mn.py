...
from dataset_frame_diff_mn import HDF5VideoDataset, collate_fn
...
def evaluate(model, val_loader, device, distillation_loss_mode, classification_loss):
    model.eval()
    ...
    with torch.no_grad():
        for batch in val_bar:
            ...
            diff_frames = batch["frame_diff"].to(device)
            labels = batch["labels"].to(device)

            # Forward
            student_emb, student_emb_for_distill, logits = model(diff_frames)

            distill_loss = distillation_loss(student_emb_for_distill, embeddings_gt[:, :-1, :], mode=distillation_loss_mode)
            class_loss = classification_loss(logits, labels.argmax(dim=1))
...
def train(args):
    ...
    train_dataset = HDF5VideoDataset(clip_embeddings_dir=args.train_hdf5_path, frame_diff_videos_dir=args.frame_diff_videos_dir, sequence_length=args.sequence_length)
    ...
    val_dataset = HDF5VideoDataset(clip_embeddings_dir=args.val_hdf5_path, frame_diff_videos_dir=args.frame_diff_videos_dir, sequence_length=args.sequence_length)
    ...
    # === Model, optimizer ===
    model = FrameDiffStudentModel(clip_model_name="ViT-B/32", device=device, num_classes=args.num_classes, alpha=args.residual_alpha).to(device)
    ...
    for epoch in range(args.epochs):
        ...
        for batch in train_bar:
            ...
            diff_frames = batch["frame_diff"].to(device)  # shape (B, T-1, 3, H, W)
            labels = batch["labels"].to(device)  # shape (B, num_classes)

            # Student forward
            student_embeddings, student_embeddings_for_distillation, logits = model(diff_frames)  # (B, T-1, embed_dim), (B, T-1, embed_dim), (B, num_classes)
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Train diff_frame-only student model")
    ...
    parser.add_argument("--num-classes", type=int, default=12, help="Number of action classes in the dataset.")
    ...
    parser.add_argument(
        "--train-hdf5-path",
        type=str,
        default="/mnt/Data/mrt/mammalnet/embeddings/mn_train_clip_vit32.h5",
        help="Path to the HDF5 file with CLIP embeddings for the training split.",
    )
    parser.add_argument(
        "--val-hdf5-path",
        type=str,
        default="/mnt/Data/mrt/mammalnet/embeddings/mn_val_clip_vit32.h5",
        help="Path to the HDF5 file with CLIP embeddings for the validation split.",
    )
    parser.add_argument(
        "--frame-diff-videos-dir",
        type=str,
        default="/mnt/Data/enz/mammalnet/frame_diff_videos",
        help="Directory containing frame difference videos.",
    )
