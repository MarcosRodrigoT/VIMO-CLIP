...
from torchmetrics.classification import MultilabelAveragePrecision, Accuracy
from tqdm import tqdm
from data.dataset_frame_diff import HDF5VideoDataset, collate_fn_pad
...
class ModelTrainer:
    def __init__(self, model, train_loader, val_loader, config):
        ...
        self.metric = MultilabelAveragePrecision(num_labels=self.config.num_classes, average="micro").to(self.config.device) # AnimalKingdom
        self.best_val_metric = 0.0  # Track the highest val_metric
        ...
        self.criterion = nn.BCEWithLogitsLoss()  # AnimalKingdom
    ...
    def train_epoch(self, epoch):
        ...
        for batch_idx, batch in enumerate(progress_bar):
            ...
            frame_diff_input = batch["frame_diff_embeddings"].to(self.config.device)
            labels = batch["labels"].to(self.config.device)
            mask_rgb = batch["mask_rgb"].to(self.config.device)  # (B, T_rgb_max)
            mask_frame_diff = batch["mask_frame_diff"].to(self.config.device)  # (B, T_frame_diff_max)

            self.optimizer.zero_grad()
            output = self.model(video_input, frame_diff_input, mask_rgb=mask_rgb, mask_flow=mask_frame_diff)
            ...
            self.metric.update(output, labels.to(dtype=torch.int))

            # Logging de batch
            if batch_idx % 50 == 0:
                mAP = self.metric.compute()  # Obtener mAP acumulado hasta ahora
                progress_bar.set_postfix({"loss": loss.item(), "mAP": f"{mAP:.4f}"})

        epoch_mAP = self.metric.compute()
    ...
    def validate(self, epoch):
        ...
        with torch.no_grad():
            for batch_idx, batch in enumerate(progress_bar):
                ...
                labels = batch["labels"].to(self.config.device)
                mask_rgb = batch["mask_rgb"].to(self.config.device)  # (B, T_rgb_max)
                mask_frame_diff = batch["mask_frame_diff"].to(self.config.device)  # (B, T_flow_max)
        ...
        self.writer.add_scalar("mAP/val", val_metric, epoch)

        return val_loss, val_metric

    def save_checkpoint(self, val_loss, val_metric, epoch, best=False):
        state = {
            "epoch": epoch,
        ...
        if val_metric > self.best_val_metric:
            ...
            logging.info(f"New best model in epoch {epoch} (mAP={val_metric:.4f}) saved to {best_save_path}")
    ...
    def train(self):
        ...
        for epoch in range(self.config.epochs):
            ...
            self.save_checkpoint(val_loss, val_metric, epoch)
            ...
            # Loggear métricas
            logging.info(f"Train Loss: {train_loss:.4f} | Train mAP: {train_mAP:.4f} | Val Loss: {val_loss:.4f} | Val mAP: {val_metric:.4f} | LR: {current_lr:.2e}")
...
class ModelTester:
    def __init__(self, model, val_loader, config):
        ...
        self.val_loader = val_loader
        self.config = config
        self.metric = MultilabelAveragePrecision(num_labels=self.config.num_classes, average="micro").to(self.config.device)
    ...
    def evaluate(self, save_predictions=False, top_k=5):
        ...
        with torch.no_grad():
            for batch in tqdm(self.val_loader, desc="Evaluando"):
                ...
                # modo de inferencia
                outputs = self.model(video_input, frame_diff_input, mask_rgb=mask_rgb, mask_flow=mask_frame_diff)

                loss = self.criterion(outputs, labels)
                total_loss += loss.item()

                self.metric.update(outputs, labels.to(dtype=torch.int))
        ...
        results["metrics"]["mAP"] = self.metric.compute().item()
...
if __name__ == "__main__":
    ...
    config.frame_diff_dataset_path = data_cfg["frame_diff_dataset_path"]
    ...
    val_dataset = HDF5VideoDataset(config.val_dataset_path, config.frame_diff_dataset_path, num_frames=None, max_frames=None)
    ...
    if config.mode in ["train", "both"]:
        train_dataset = HDF5VideoDataset(config.train_dataset_path, config.frame_diff_dataset_path, num_frames=None, max_frames=None)
