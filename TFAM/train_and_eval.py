...
from data.dataset_amo_clip import HDF5VideoDataset, collate_fn_pad
...
class ModelTrainer:
    def __init__(self, model, train_loader, val_loader, config):
        ...
        self.mAP_metric = MultilabelAveragePrecision(
        ...
        self.scheduler = lr_scheduler.CosineAnnealingLR(
            self.optimizer,
            T_max=self.config.epochs,  # Ciclo completo sobre todas las épocas
            eta_min=1e-6               # Learning rate mínimo (opcional, por defecto es 0)
        )
...
class ModelTester:
            ...
            for batch in tqdm(self.val_loader, desc="Evaluando"):
                ...
                for i in range(outputs.size(0)):
                    ...
                    for idx in sorted_indices[:top_k]:
                        video_result['predictions'][str(idx)] = {
                            'class_name': self.class_names.get(str(idx), f"class_{idx}"),
                            'probability': round(float(probs_i[idx]), 4)
                        }

                    # Almacenar etiquetas verdaderas
                    true_labels = np.where(labels[i].cpu().numpy() == 1)[0]
                    for lbl in true_labels:
                        video_result['true_labels'].append({
                            'class_id': str(lbl),
                            'class_name': self.class_names.get(str(lbl), f"class_{lbl}")
                        })
        ...
        for video in results['videos'][:3]:
            ...
            for cls_id, pred in video['predictions'].items():
                table_data.append([
                    ...
                    "Yes" if any(lbl['class_id'] == cls_id for lbl in video['true_labels']) else "No"
                ])

            print(tabulate(table_data,
                           headers=['Class', 'Probability', 'Correct'],
                           tablefmt='pretty'))
    ...
    def run_full_evaluation(self):
        ...
        ], headers=['Modo', 'Pérdida', 'mAP'], tablefmt='grid'))
...
if __name__ == "__main__":
    ...
    val_dataset = HDF5VideoDataset(
        config.val_dataset_path,
        config.flow_dataset_path,
        num_frames=None,
        max_frames=None
    )
